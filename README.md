...
| 演出日历 | 直连 Fantopia 拉取待开售演出，**以场次为单位**按 `sellStartTime` 铺进月历（会员购 / 公开发售分开标注），带开售倒计时与一键跳转购票页；默认合并全部地区（`area`）的活动池，可切单地区；点日历上的日期看当天开售详情，**同一活动同一天多场只出一张卡**（当天各场次以开售日程块列出） | `tools/event-calendar.html` |
