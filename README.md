...
| 演出日历 | 直连 Fantopia 拉取演出数据，支持**两个维度**看同一个日历：**开售日程**（以场次为单位按 `sellStartTime` 铺，蓝系点：公开发售实心 / 会员购空心 / 缺货登记灰 / 已开售浅灰）与**演出日程**（按每个场次的**演出日期**铺，橙系点：未开卖实心 / 已在售空心 / 已过浅灰，同场多批次开售只算一场）—— **切维度 = 换色系**，一眼可辨。扫**全部未结束活动**（不只「待开售」活动 —— 热卖中的活动里也常藏着还没开售的加场）；命中本地缓存时**先秒开上次结果再后台静默刷新**（顶栏进度线 + 首次访问骨架屏）；默认合并全部地区（`area`）的活动池，可切单地区；日历格用**场次点阵**（一场一点，超 4 场 `+N`）表达密度；图例分「日程 / 日期」两组：日程点色随维度换，**中国法定节假日 / 调休上班日**恒定标注；点日期看当天详情，**同一活动同一天多场只出一张卡**；带倒计时与一键跳转购票页 | `tools/event-calendar.html` |
